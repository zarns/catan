        ...
        @if (!loading) {
          ...
          <div class="player-count-section">
            ...
            <div class="button-group">
              <button
                class="player-button"
                [class.selected]="numPlayers === 2"
                (click)="setNumPlayers(2)"
              >
                2 PLAYERS
              </button>
              <button
                class="player-button"
                [class.selected]="numPlayers === 3"
                (click)="setNumPlayers(3)"
              >
                3 PLAYERS
              </button>
              <button
                class="player-button"
                [class.selected]="numPlayers === 4"
                (click)="setNumPlayers(4)"
              >
                4 PLAYERS
              </button>
            </div>
  ...
  styles: [
    `
      ...
      .logo {
        ...
        color: #f3ea15;
        text-shadow:
          0 0 15px rgba(243, 234, 21, 0.5),
          0 0 20px rgba(243, 234, 21, 0.2);
        letter-spacing: 4px;
        transform: skew(-5deg);
      }
      ...
      .player-button.selected {
        ...
        box-shadow:
          0 0 15px rgba(243, 234, 21, 0.5),
          inset 0 0 10px rgba(243, 234, 21, 0.3);
      }
...
export class HomeComponent implements OnInit {
  ...
  async startGame(gameMode: GameMode) {
    console.log(
      '🎮 HomeComponent: Starting game with mode:',
      gameMode,
      'players:',
      this.numPlayers
    );
