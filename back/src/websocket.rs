...
use crate::manager::{Action, ActionResult, GameManager};
use crate::{CatanError, CatanResult};
...
impl WebSocketManager {
    ...
    // Create a new game
    pub async fn create_game(&self, num_players: u8, bot_type: &str) -> CatanResult<String> {
                        ...
                        if let Ok(ws_msg) = serde_json::from_str::<WsMessage>(&text) {
                            match ws_msg {
                                WsMessage::PlayerAction(action) => {
                                    ...
                                    let _player_id = "player_0".to_string(); // In reality would come from auth

                                    // Process player action
                                    let result = {
                                        let game_managers_guard = game_managers.lock().await;
                                        if let Some(game_manager) =
                                            game_managers_guard.get(&game_id_for_receive)
                                        {
                                            let mut game_manager_lock = game_manager.lock().await;
                                            let player_idx = 0; // For now, hardcoded as the first player
                                            game_manager_lock
                                                .process_action(player_idx, action.clone())
                                        } else {
                                            Err(crate::manager::GameError::GameNotFound(
                                    ...
                                    match result {
                                        Ok(action_result) => {
                                            ...
                                            if action_result.success {
                                                if let Ok(updated_state) = websocket_manager
                                                    .get_game_state(&game_id_for_receive)
                                                    .await
                                                {
                                                    let _ = tx_clone.send((
                                                        game_id_for_receive.clone(),
        ...
        while bot_turn {
            ...
            match result {
                Ok((action, action_result)) => {
                    // Send the bot action
                    let _ = tx.send((game_id.to_string(), WsMessage::BotAction(action)));

                    // Send the action result
                    let _ = tx.send((game_id.to_string(), WsMessage::ActionResult(action_result)));

                    // Send updated game state
                    if let Ok(updated_state) = self.get_game_state(game_id).await {
                        let _ = tx.send((game_id.to_string(), WsMessage::GameState(updated_state)));
                    }
                }
                Err(e) => {
